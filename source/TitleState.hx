...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		abduSprite = new FlxSprite(0, FlxG.height * 0.52).loadGraphic(Paths.image('abdu_white'));
		add(abduSprite);
		abduSprite.visible = false;
		abduSprite.setGraphicSize(Std.int(abduSprite.width * 0.8));
		abduSprite.updateHitbox();
		abduSprite.screenCenter(X);
		abduSprite.antialiasing = ClientPrefs.globalAntialiasing;
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			...
			if(pressedEnter)
			{
				...
				bump(FlxG.camera, "zoom", 1.6, 1, 1.2);
	...
	function createCoolText(textArray:Array<String>, ?offset:Float = 0)
	{
		...
		{
			// why tf is it called money ?! -Abdoo
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 6:
					...
					#end
				case 8:
					//FlxTween.tween(FlxG.camera, {zoom: 1.5}, 0.23, {ease: FlxEase.cubeOut});
					addMoreText('this mf', -40);
					abduSprite.visible = true;
					new FlxTimer().start(0.1, function(tmr:FlxTimer) {
						bump(abduSprite.scale, "x", 1.2, 1, 0.16);
						bump(abduSprite.scale, "y", 1.2, 1, 0.16);
					});
				...
				// credTextShit.visible = true;
				case 12:
					FlxTween.tween(FlxG.camera, {zoom: 1.3}, 0.2, {ease: FlxEase.circOut});
				...
				case 24:
					abduSprite.loadGraphic(Paths.image('pedro', 'preload'));
					abduSprite.screenCenter(Y);
					abduSprite.x = FlxG.height *  0.6;
					abduSprite.visible = true;
					bump(abduSprite.scale, "x", 1.2, 1, 0.15);
					bump(abduSprite.scale, "y", 1.2, 1, 0.15);
				case 25:
					abduSprite.visible = false;
					remove(abduSprite);
			}
		...
		if (!skippedIntro)
		{
			if (playJingle) //Ignore deez
			{
				...
				switch(easteregg)
				{
					...
					case 'ABDOO':
						FlxG.sound.play(Paths.music('bwekfest', 'shared'), 1, true);
