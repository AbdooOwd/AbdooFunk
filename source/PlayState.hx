...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (needChars[1]) add(gfGroup); //Needed for blammed lights

		// Shitty layering but whatev it works LOL
		if (curStage == 'limo')
			add(limo);

		if (needChars[0]) add(dadGroup);
		if (needChars[2]) add(boyfriendGroup);

		overOver = new FlxSprite(FlxG.width * -0.5, FlxG.height * -0.5);
	...
	function dzCutscene() {
		...
		cutsceneHandler.timer(4, function(){
			camFollow.set(dad.getMidpoint().x + 100, boyfriend.getMidpoint().y);
		});
		...
		switch(eventName) {
			...
			case 'Blackout':
				...
				/*
				   	...
				   	if value2 is a string then we'll use val2str in the event
					but if value2 is a float then we'll use val2 in the event
				*/

				if (val1 <= 0) val1 = 0.75;

				var tempo_val: String = value2.toLowerCase().trim();

				if ( !(tempo_val == "on" || tempo_val == "off") ) {
					val2 = Std.parseFloat(value2);
					if (val2 <= 0) val2 = 1;
					isSwitch = false;
				} else {
					isSwitch = true;
					val2str = value2.toLowerCase().trim();
				}
				/*
					if we find DO NOT find a string in value2 then it's a float
					but if we DO find a string then it's a switch event (isSwitch will be true)
				*/

				overBg.makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.BLACK);
				overOver.makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.BLACK);


				if ((isSwitch == true && val2str == "on") || val2 > 0) {
